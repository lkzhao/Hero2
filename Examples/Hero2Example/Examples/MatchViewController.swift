...
class MatchViewController: ComponentViewController {
  var shapes: [Shape] = [
    Shape(
      name: "rectangle", color: .systemPurple,
      transform: .identity.translatedBy(x: 100, y: -200).scaledBy(x: 0.4, y: 0.4).rotated(by: .pi * 1.1)),
    Shape(
      name: "square", color: .systemRed,
      transform: .identity.translatedBy(x: -100, y: -200).scaledBy(x: 0.7, y: 0.7).rotated(by: .pi * -0.33)),
    Shape(
      name: "circle", color: .systemGreen, transform: .identity.translatedBy(x: 50, y: 200).scaledBy(x: 1.1, y: 1.1)),
...
class MatchDetailViewController: ComponentViewController {
  ...
  override var component: Component {
    VStack(spacing: 20) {
      ZStack {
        Image(shape.image).size(width: 200, height: 200).heroID(shape.id).tintColor(shape.color).contentMode(
          .scaleAspectFit)
