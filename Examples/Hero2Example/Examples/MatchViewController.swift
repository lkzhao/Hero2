...
class MatchViewController: ComponentViewController {
    var shapes: [Shape] = [
        ...
        Shape(
            name: "circle",
            color: .systemGreen,
            transform: .identity.translatedBy(x: 50, y: 200).scaledBy(x: 1.1, y: 1.1)
        ),
    ...
    override var component: Component {
        VStack(spacing: 8) {
            ZStack {
                for shape in shapes {
                    Image(shape.image).tintColor(shape.color).contentMode(.scaleAspectFit).heroID(shape.id)
                        .heroModifiers([
                            .whenNotMatched(.fade)
                        ])
...
class MatchDetailViewController: ComponentViewController {
    ...
    override var component: Component {
        VStack(spacing: 20) {
            ...
            .size(width: .fill, height: .aspectPercentage(14 / 9))
            .tappableView {
                $0.dismiss()
            }
